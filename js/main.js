(function () {
  ...
  const defaultSeminarEndTime = '11:50:00';
  const defaultSeminarTimeLabel = '11:00–11:50 (UTC+8)';
  const defaultSeminarLocation = 'Lecture Hall B';
  const defaultZoomLink = 'https://hkust-gz-edu-cn.zoom.us/j/96951141900?pwd=QrbBaW1aMGvkgZqCpaJkoiDteIhUwf.1';
  const defaultZoomId = '969 5114 1900';
  const defaultZoomPasscode = 'ait';
  ...
  function getSessionEnd(session) {
    ...
    const endTime = session.getAttribute('data-end-time') || defaultSeminarEndTime;
    const sessionEnd = new Date(`${dateAttr}T${endTime}${seminarTimeZoneOffset}`);
  ...
  function renderFeaturedPanel(session) {
    ...
    const timeLabel = session.getAttribute('data-time-label') || defaultSeminarTimeLabel;
    const location = session.getAttribute('data-location') || defaultSeminarLocation;
    const zoomLink = session.getAttribute('data-zoom-url') || defaultZoomLink;
    const zoomId = session.getAttribute('data-zoom-id') || defaultZoomId;
    const zoomPasscode = session.getAttribute('data-zoom-passcode') || defaultZoomPasscode;
    const placeholderInitials = getInitials(speakerName);
    const speakerVisual = imgSrc
      ? `<img src="${imgSrc}" alt="${speakerName || imgAlt}" class="h-full w-full object-cover" onerror="this.classList.add('hidden'); this.nextElementSibling.classList.remove('hidden');">
    ...
    container.innerHTML = `
      <article class="featured-layout overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm card-hover dark:border-slate-800 dark:bg-slate-950">
        ...
        <div class="flex flex-col justify-center p-6 lg:p-8">
          ...
          <p class="meeting-access">Zoom ID: ${zoomId} · Passcode: <strong>${zoomPasscode}</strong></p>
        </div>
      </article>
    `;
